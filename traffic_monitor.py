from ryu.base import app_manager
from ryu.controller import ofp_event
from ryu.controller.handler import CONFIG_DISPATCHER, MAIN_DISPATCHER, DEAD_DISPATCHER
from ryu.controller.handler import set_ev_cls
from ryu.ofproto import ofproto_v1_3
from ryu.lib.packet import packet, ethernet   #extracts paclet address
from ryu.lib import hub #bg thread monitor 
#ryu applications handels flow events and stats , config setup and other operations 
class TrafficMonitor(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION] # inform switch to use openflow version 

    def __init__(self, *args, **kwargs):
        super(TrafficMonitor, self).__init__(*args, **kwargs)
        self.datapaths = {} # store the switches that are connected to controller 
        self.mac_to_port = {}  #stores port
        self.monitor_thread = hub.spawn(self.monitor) #starts bg monitor loop and runs every few seconds 

    # Registeration of  switch
    @set_ev_cls(ofp_event.EventOFPStateChange, [MAIN_DISPATCHER, DEAD_DISPATCHER]) 
    #triggers when switches are called /connected

    #handels initial setup 
    def state_change_handler(self, ev):
        datapath = ev.datapath
        if ev.state == MAIN_DISPATCHER:
            if datapath.id not in self.datapaths:
                print(f"Register datapath: {datapath.id}")
                self.datapaths[datapath.id] = datapath
        elif ev.state == DEAD_DISPATCHER: # if switch gets disconnected 
            if datapath.id in self.datapaths:
                print(f"Unregister datapath: {datapath.id}")
                del self.datapaths[datapath.id] #remove switch

    # Monitor loop
    def monitor(self):
        while True:
            for dp in self.datapaths.values():
                self.request_stats(dp)
                #for every switch /loop asks stats
            hub.sleep(5)

    # Request flow stats
    def request_stats(self, datapath):
        ofproto = datapath.ofproto
        # used to creat openflow rules 
        parser = datapath.ofproto_parser
#creates open flow messages 
        req = parser.OFPFlowStatsRequest(datapath)
        datapath.send_msg(req)
#parsermatches packets 


    # Receive flow stats
    @set_ev_cls(ofp_event.EventOFPFlowStatsReply, MAIN_DISPATCHER)
    def flow_stats_reply_handler(self, ev):
        body = ev.msg.body  #contains flow data

        print("\n--- Flow Statistics ---")
        for stat in sorted(body, key=lambda x: (x.match.get('in_port', 0))):
            #sorts flow by input ports

            print(f"Match: {stat.match}, "
                  f"Packets: {stat.packet_count}, "
                  f"Bytes: {stat.byte_count}")

    # Packet-in handler (learning switch logic)

    # runs when packet arrives 
    @set_ev_cls(ofp_event.EventOFPPacketIn, MAIN_DISPATCHER)
    def packet_in_handler(self, ev):
        msg = ev.msg
        datapath = msg.datapath
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser

        dpid = datapath.id
        self.mac_to_port.setdefault(dpid, {})

        pkt = packet.Packet(msg.data)
        eth_pkt = pkt.get_protocol(ethernet.ethernet)  # ethernet packet 

#dest and src MAC address
        dst = eth_pkt.dst
        src = eth_pkt.src

        in_port = msg.match['in_port']

        # Learn MAC address
        self.mac_to_port[dpid][src] = in_port

        # Decide output port
        if dst in self.mac_to_port[dpid]:
            out_port = self.mac_to_port[dpid][dst]
        else:
            out_port = ofproto.OFPP_FLOOD
            #braidcasting is done iin else part

#action defines where packet goes 
        actions = [parser.OFPActionOutput(out_port)]

        # Install flow rule (if not flooding)
        if out_port != ofproto.OFPP_FLOOD:
            match = parser.OFPMatch(in_port=in_port, eth_dst=dst) # condition/protocols for parser 
            self.add_flow(datapath, 1, match, actions)

        # Send packet
        out = parser.OFPPacketOut(
        #forward packets 
            datapath=datapath,
            buffer_id=msg.buffer_id,
            in_port=in_port,
            actions=actions,
            data=msg.data if msg.buffer_id == ofproto.OFP_NO_BUFFER else None
        )
        datapath.send_msg(out)

    # Add flow rule
    #basically creates flow rules 
    def add_flow(self, datapath, priority, match, actions):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser

        inst = [parser.OFPInstructionActions(
            ofproto.OFPIT_APPLY_ACTIONS, actions)] #these apply actions

        mod = parser.OFPFlowMod(
        #flow rule object 
            datapath=datapath,
            priority=priority,
            match=match,
            instructions=inst
        )
        datapath.send_msg(mod)